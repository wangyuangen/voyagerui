...
import { dataDictStore } from '../stores/datadict'
...
export async function getBackEndControlRoutes() {
  const res = await new AuthApi().currentMenuRoute().catch(() => {})
  if (res?.success && (res?.data?.length as number) > 0) {
    ...
    res.data?.forEach((menu) => {
      menus.push({
        ...
        name: menu.routeName ? menu.routeName : menu.id + '',
        path: menu.routeUrl ? menu.routeUrl : menu.id + '',
        redirect: menu.redirectUrl,
        component: menu.view?.path ? menu.view.path : 'Layout',
        meta: {
